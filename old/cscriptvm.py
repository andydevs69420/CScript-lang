...
class STACK(object):
    def __init__(self, _size:int=1000):
        self.__internal = [
            None for _r in range(_size)
        ]
        self.__pointer = 0
    
    def push(self, _any:CSObject):
        self.__internal[self.__pointer] = _any
        self.__pointer += 1

    def pop(self):
        _top = self.__internal[self.__pointer]
        self.__pointer -= 1
        return _top
    
    def top(self):
        return self.__internal[self.__pointer - 1]
...
def cs__define_prop(_env:CSXEnvironment, _name:str, _csobject:CSObject):
    """ Defines a property inide scope
...
def cs__rot_2(_env:CSXEnvironment):
    """
        b ----->   a
        a          b
    """
    _top = _env.stack.poll()
    _nxt = _env.stack.poll()
...
# ======================== OBJECT CREATION|
# ========================================|
def cs__set_prototype(_env:CSXEnvironment, _object:CSObject, _prototoype:CSObject):
    _object.put("__proto__", _prototoype)
...
def cs__new_number(_env:CSXEnvironment, _raw_py_number:int|float):
    ...
    cs__set_prototype(_env, _env.stack.top(), cs__get_prototype(_env, _env.stack.top().type))
...
def cs__raw_call(_env:CSXEnvironment, _csobject:CSFunction|CSNativeFunction, _arg_count:int):
    ...
    assert _csobject.get("argc").this == _arg_count, "insufficient argument count %s (%s != %d) !" % (_csobject.get("name").__str__(), _csobject.get("argc").__str__(), _arg_count)
...
def cs__has_method(_env:CSXEnvironment, _top_object:CSObject, _method:str):
    """ Checks if object has method
        ...
        _method : str
    """
    if  not _top_object.hasKey("__proto__"):
        return False

    _proto = _top_object.get("__proto__")
    if  cs__has_attribute(_env, _proto, _method):
        # check if callable
        return cs__is_callable(_proto.get(_method))
...
def cs__get_method(_env:CSXEnvironment, _top_object:CSObject, _attr:str):
    ...
    # push
    _env.stack.push(_proto.get(_attr))
...
def cs__call(_env:CSXEnvironment, _code:csrawcode):
    ...
    while _ipointer < len(_code.code) and (not _returned):
        ...
        match _instruction.opcode:
            ...
            # this
            case CSOpCode.THIS_OP:
                print("THIS:", _env.stack.peek())
                ...
            ...
            case CSOpCode.GET_METHOD:
                ...
                if  not cs__has_method(_env,_env.stack.peek(), _attribute):
                    cs__error(
                        _env, ("AttributeError: %s has no method %s!" % (_env.stack.peek().__str__(), _attribute)),
                        _instruction.get("loc")
                    )
                    continue
            ...
            case CSOpCode.BINARY_POW:
                _a = _env.stack.poll()
                if  not cs__has_method(_env, _a, "__pow__"):
                    cs__error(
                        _env, ("TypeError: invalid operation (%s) of operands!" % _instruction.get("opt")),
                        _instruction.get("loc")
                    )
                ...
                ######## invoke self __pow__ method
                cs__invoke_method(_env, _a, "__pow__", 1)
            ...
            case CSOpCode.BINARY_ADD:
                ...
                cs__invoke_method(_env, _env.stack.peek(), "__add__", 1)
            ...
            case CSOpCode.COMPARE_OP:
                match _instruction.get("opt"):
                    ...
                    case "==":
                        ...
                        # check rhs
                        if  not cs__has_method(_env, _env.stack.peek(), "__eq_" + _a.type + "__"):
                            _env.stack.poll()
                            cs__new_boolean(_env, False)
                            continue
                    ...
                    case "!=":
                        ...
                        # check rhs
                        if  not cs__has_method(_env, _env.stack.peek(), "__neq_" + _a.type + "__"):
                            _env.stack.poll()
                            cs__new_boolean(_env, True)
                            continue
            ...
            case CSOpCode.INPLACE_ADD:
                _a = _env.stack.poll()
                _iadd_method = "__iadd_" + _env.stack.peek().type + "__"
                if  not cs__has_method(_env, _a, _iadd_method):
                    cs__error(
                        _env, ("TypeError: invalid operation (%s) of operands!" % _instruction.get("opt")),
                        _instruction.get("loc")
                    )
                ...
                ######## invoke self __iadd_RHS__ method
                cs__invoke_method(_env, _a, _iadd_method, 1)
            ...
            case CSOpCode.PRINT_OBJECT:
                ...
                for idx in range(_size):
                    _string  = _env.stack.pop().__str__()
